...
export async function exportPrintLayoutToPdf(
  ...
  try {
    ...
    const pageEls = Array.from(clone.querySelectorAll<HTMLElement>('.pdf-page'));
    ...
    for (const el of targets) {
      ...
      while (yOffsetPx < canvas.height) {
        ...
        ctx.drawImage(canvas, 0, yOffsetPx, canvas.width, h, 0, 0, slice.width, h);

        if (!firstPdfPage) pdf.addPage();
        pdf.addImage(slice.toDataURL('image/jpeg', 0.95), 'JPEG', 0, 0, pageWidthMm, h * mmPerPx);
        firstPdfPage = false;
