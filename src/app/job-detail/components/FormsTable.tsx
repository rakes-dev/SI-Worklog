...
export default function FormsTable({ job, onToast }: FormsTableProps) {
  ...
  const [sortKey, setSortKey] = useState<
    "position" | "name" | "date" | "area" | "amount"
  >("position");
  ...
  const moveForm = async (formId: string, dir: "up" | "down") => {
    ...
    const targetId =
      dir === "up" ? activeIds[activeIdx - 1] : activeIds[activeIdx + 1];
    if (!targetId) return;
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      onToast(
        "success",
        "Form deleted",
        "Form moved to trash. You can restore it anytime.",
      );
    } catch (error) {
  ...
  const handlePermanentDelete = async () => {
    ...
    try {
      ...
      onToast(
        ...
        "The form has been removed permanently.",
      );
    } catch (error) {
      ...
      onToast(
        "error",
        "Delete failed",
        "Could not permanently delete this form.",
      );
    } finally {
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-border">
        <div>
          <h2 className="font-semibold text-foreground">
            Standard Interior Forms
          </h2>
          <p className="text-xs text-muted-foreground mt-0.5">
            {activeForms.length} active form
            {activeForms.length !== 1 ? "s" : ""} · {deletedForms.length} in
            trash
          </p>
        </div>
        <button
      ...
      {activeForms.length === 0 && deletedForms.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center px-4">
          ...
          <p className="text-sm text-muted-foreground max-w-xs">
            Add a Standard Interior form to start recording measurements and
            summary data for this job.
          </p>
          <button
            onClick={() => setShowTemplateModal(true)}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-secondary/50">
                  <th className="text-left px-4 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wide w-10">
                    #
                  </th>
        ...
        <div className="border-t border-border">
          <div className="px-5 py-3 bg-secondary/30 border-b border-border">
            <h3 className="font-medium text-foreground text-sm">
              Deleted Forms ({deletedForms.length})
            </h3>
            <p className="text-xs text-muted-foreground mt-0.5">
              These forms are soft-deleted. Restore them to recover.
            </p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-secondary/50">
                  ...
                  <th className="text-right px-4 py-2 text-xs font-medium text-muted-foreground uppercase tracking-wide w-20">
                    Actions
                  </th>
                </tr>
              </thead>
