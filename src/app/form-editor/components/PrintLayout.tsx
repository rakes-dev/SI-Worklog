...
export default function PrintLayout({ form, job }: PrintLayoutProps) {
  ...
  const visibleSummaryRows = form.summaryRows.filter(
    (row) => !isEmptySummaryRow(row),
  );
  const visibleMeasurementRows = form.measurementRows.filter(
    (row) => !isEmptyMeasurementRow(row),
  );

  // Unit label for the Total Area column across all visible measurement rows.
  const areaHeaderUnit = aggregateAreaUnitLabel(
  ...
  const firstPageLimit = hasSummaryRows
    ? Math.max(
        1,
        FIRST_PAGE_MAX_ROWS_WITH_SUMMARY - locationDeduction - summaryDeduction,
      )
            ...
            <div style={{ flex: 1 }}>
              ...
              {isFirstPage && (
                <>
                  ...
                  {/* Header Info Table */}
                  <table
                    className="print-clean-table"
                    style={{ marginBottom: "10pt", fontSize: "8pt" }}
                  >
                    <tbody>
                      <tr>
                        <td
                      ...
                      <tr>
                        ...
                        <td style={{ padding: "2px", paddingLeft: "4px" }}>
                          {form.totalSheets}
                        </td>
                        <td
                          style={{
