  ...
  const handleSaveForm = async (e: React.FormEvent) => {
    ...
    if (!formItem.arc_no.trim() || !formItem.description.trim()) {
      addToast(
        "warning",
        "Validation Error",
        "ARC No. and Description are required.",
      );
      return;
    }
    ...
    try {
      ...
      addToast(
        "success",
        "ARC Item Saved",
        `"${formItem.arc_no}" has been saved successfully.`,
      );
      await loadItems();
      setView("list");
    } catch (error) {
      ...
      try {
        ...
        if (arcIdx === -1) {
          addToast(
            "error",
            "Invalid CSV",
            'CSV must contain an "arc_no" column.',
          );
          return;
        }
        ...
        addToast(
          "success",
          "CSV Imported",
          `${parsed.length} rows imported successfully.`,
        );
        await loadItems();
      } catch (error) {
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-border bg-secondary/50">
                        <th className="px-4 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wide text-left w-10">
                          #
                        </th>
                        <th className="px-4 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wide text-left min-w-[120px]">
                          ARC No.
                        </th>
                        <th className="px-4 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wide text-left min-w-[140px]">
                          Job Type
                        </th>
                        <th className="px-4 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wide text-left min-w-[80px]">
                          Coat
                        </th>
                        <th className="px-4 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wide text-left min-w-[250px]">
                          Description
                        </th>
                        <th className="px-4 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wide text-right min-w-[100px]">
                          Final Rate (₹)
                        </th>
                        <th className="px-4 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wide text-left min-w-[80px]">
                          UOM
                    ...
                    <tbody>
                      {processedItems.map((item, idx) => (
                        ...
                        >
                          <td className="px-4 py-3 text-xs text-muted-foreground font-tabular text-center">
                            {idx + 1}
                          </td>
                          ...
                          <td
                            className="px-4 py-3 text-muted-foreground truncate max-w-[250px]"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormItem({ ...formItem, arc_no: e.target.value })
                      }
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormItem({ ...formItem, job_type: e.target.value })
                      }
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <input
                      ...
                      value={formItem.coat}
                      onChange={(e) =>
                        setFormItem({ ...formItem, coat: e.target.value })
                      }
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div className="flex flex-col gap-1 sm:col-span-2">
                  ...
                  <span className="text-base font-bold text-primary">
                    {selectedItem.final_rate !== ""
                      ? `₹${Number(selectedItem.final_rate).toFixed(2)}`
                      : "—"}
                  </span>
                </div>
