...
export default function AppLayout({ children }: AppLayoutProps) {
  ...
  const deciding = status === 'loading' || (status === 'authenticated' && authorized === null);
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-background p-6 text-center">
        ...
        <p className="text-sm text-muted-foreground mt-2 max-w-md">
          Your email <span className="font-medium text-foreground">{user?.email}</span> has not been
          added to the access list. Ask an admin to add it in the Admin panel before you can use the
          app.
        </p>
        {accessError && <p className="text-xs text-amber-600 mt-3 max-w-md">{accessError}</p>}
        <button
          onClick={logOut}
